            ...
            .then(async (response) => {
              ...
              data.miners.map((miner, key) => {
                ...
                const minerTemplate = {
                  ...
                  Software: miner.userAgent === null ? "unknown, update your miner" : miner.userAgent,
        ...
        if (
          ...
          return new Promise((resolve, reject) => {
            ...
            .then(async (response) => {
              ...
              const responseData = [
                response.name && response.name.trim() ? {title: "Username", value: response.name, type: "info"} : null,
                {title: "Miner address", value: response.addressRS, type: "info"},
                {title: "Pending Balance", value: response.pendingBalance, type: "info"},
                {title: "Current Deadline", value: response.currentDeadline || "Waiting...", type: "info"},
                {title: "Past deadlines", value: "BookmarkedMinerDeadlines", type: "BookmarkedMinerDeadlines"},
                {title: "Confirmed Deadlines", value: confirmedDeadlines || "", type: "info"},
                {title: "Effective Capacity", value: response.effectiveCapacity, type: "info"},
                {title: "Physical Capacity", value: response.physicalCapacity, type: "info"},
                {title: "PoC+ Boost", value: response.poCPlus, type: "info"},
                {title: "Committed Balance", value: response.committedBalance, type: "info"},
                {title: "Donation Percent", value: response.donationPercent + "%", type: "info"},
                {title: "Pool Share", value: (parseFloat(response.share) * 100).toFixed(3) + "%", type: "info"},
                {title: "Software", value: response.Software, type: "info"},
                {label: "Delete bookmark", type: "action"},
              ];
